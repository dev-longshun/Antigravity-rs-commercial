...
import { AppConfig, ProxyConfig, StickySessionConfig, ExperimentalConfig, ZeaburStatus } from '../types/config';
...
export default function ApiProxy() {
    ...
    // Zeabur 云端 states
    const [publicAccessTab, setPublicAccessTab] = useState<'zeabur' | 'cloudflared'>('zeabur');
    const [zeaburStatus, setZeaburStatus] = useState<ZeaburStatus>({
        connected: false,
        accounts_synced: 0,
    });
    const [zeaburLoading, setZeaburLoading] = useState(false);
    const [zeaburInstanceUrl, setZeaburInstanceUrl] = useState('');
    const [zeaburApiKey, setZeaburApiKey] = useState('');
    const [zeaburAutoSync, setZeaburAutoSync] = useState(false);
    const [zeaburSyncInterval, setZeaburSyncInterval] = useState(5); // 分钟
    ...
    useEffect(() => {
        ...
        const zeaburInterval = setInterval(loadZeaburStatus, 10000);
        return () => {
            clearInterval(interval);
            clearInterval(cfInterval);
            clearInterval(zeaburInterval);
        };
    }, []);

    // Zeabur: 从 appConfig 同步本地状态
    useEffect(() => {
        if (appConfig?.zeabur) {
            setZeaburInstanceUrl(appConfig.zeabur.instance_url || '');
            setZeaburApiKey(appConfig.zeabur.api_key || '');
            setZeaburAutoSync(appConfig.zeabur.auto_sync || false);
            setZeaburSyncInterval(Math.round((appConfig.zeabur.sync_interval_secs || 300) / 60));
        }
    }, [appConfig?.zeabur]);
    ...
    // Zeabur: 加载状态
    const loadZeaburStatus = async () => {
        try {
            const status = await invoke<ZeaburStatus>('zeabur_get_status');
            setZeaburStatus(status);
        } catch {
            // 忽略错误
        }
    };
    ...
    // Zeabur: 测试连接
    const handleZeaburTestConnection = async () => {
        if (!zeaburInstanceUrl.trim()) return;
        setZeaburLoading(true);
        try {
            await invoke<boolean>('zeabur_test_connection', {
                url: zeaburInstanceUrl.trim(),
                apiKey: zeaburApiKey.trim(),
            });
            showToast(t('proxy.zeabur.connected'), 'success');
            await loadZeaburStatus();
        } catch (error) {
            showToast(t('proxy.zeabur.connect_failed', { error: String(error) }), 'error');
        } finally {
            setZeaburLoading(false);
        }
    };

    // Zeabur: 连接
    const handleZeaburConnect = async () => {
        if (!zeaburInstanceUrl.trim() || !zeaburApiKey.trim()) return;
        setZeaburLoading(true);
        try {
            const config = {
                enabled: true,
                instance_url: zeaburInstanceUrl.trim(),
                api_key: zeaburApiKey.trim(),
                auto_sync: zeaburAutoSync,
                sync_interval_secs: zeaburSyncInterval * 60,
            };
            const status = await invoke<ZeaburStatus>('zeabur_connect', { config });
            setZeaburStatus(status);
            // 持久化配置
            if (appConfig) {
                saveConfig({ ...appConfig, zeabur: config });
            }
            showToast(t('proxy.zeabur.connected'), 'success');
        } catch (error) {
            showToast(t('proxy.zeabur.connect_failed', { error: String(error) }), 'error');
    ...
    // Zeabur: 断开
    const handleZeaburDisconnect = async () => {
        setZeaburLoading(true);
        try {
            const status = await invoke<ZeaburStatus>('zeabur_disconnect');
            setZeaburStatus(status);
            if (appConfig) {
                saveConfig({ ...appConfig, zeabur: { ...appConfig.zeabur, enabled: false } });
            }
            showToast(t('proxy.zeabur.disconnected'), 'info');
        } catch (error) {
            showToast(String(error), 'error');
        } finally {
            setZeaburLoading(false);
    ...
    // Zeabur: 立即同步
    const handleZeaburSync = async () => {
        setZeaburLoading(true);
        try {
            const status = await invoke<ZeaburStatus>('zeabur_sync_accounts');
            setZeaburStatus(status);
            showToast(t('proxy.zeabur.sync_success'), 'success');
        } catch (error) {
            showToast(t('proxy.zeabur.sync_failed', { error: String(error) }), 'error');
        } finally {
            setZeaburLoading(false);
        }
    };
    ...
    // Zeabur: 复制公网URL
    const handleZeaburCopyUrl = async () => {
        if (zeaburInstanceUrl) {
            const success = await copyToClipboard(zeaburInstanceUrl);
            if (success) {
                setCopied('zeabur-url');
                setTimeout(() => setCopied(null), 2000);
                showToast(t('proxy.zeabur.url_copied'), 'success');
            }
        }
    };
    ...
    const handleZeaburCopyConfig = async () => {
        ...
        if (success) {
            ...
            setTimeout(() => setCopied(null), 2000);
            showToast(t('proxy.zeabur.config_copied'), 'success');
        }
    };

    // Zeabur: 保存自动同步设置
    const handleZeaburSaveAutoSync = (autoSync: boolean, intervalMin?: number) => {
        const interval = intervalMin ?? zeaburSyncInterval;
        setZeaburAutoSync(autoSync);
        if (intervalMin !== undefined) setZeaburSyncInterval(intervalMin);
        if (appConfig) {
            saveConfig({
                ...appConfig,
                zeabur: {
                    ...appConfig.zeabur,
                    auto_sync: autoSync,
                    sync_interval_secs: interval * 60,
                },
            });
        }
    };
                ...
                {/* External Providers Integration */}
                {
                    !configLoading && !configError && appConfig && (
                        <div className="space-y-4">
                            ...
                            {isTauri() && (
                                <CollapsibleCard
                                    title={t('proxy.publicAccess.title', { defaultValue: 'Public Access' })}
                                    ...
                                    rightElement={
                                        zeaburLoading || cfLoading ? (
                                            <span className="loading loading-spinner loading-xs"></span>
                                        ) : zeaburStatus.connected ? (
                                            <span className="text-xs px-2 py-1 rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 flex items-center gap-1">
                                                <CheckCircle size={12} />
                                                {t('proxy.zeabur.connected')}
                                            </span>
                                ...
                                >
                                    <div className="space-y-4">
                                        {/* Tab 切换 */}
                                        <div className="flex gap-2 p-1 bg-gray-100 dark:bg-base-200 rounded-lg">
                                            <button
                                                onClick={() => setPublicAccessTab('zeabur')}
                                                className={cn(
                                                    "flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center justify-center gap-2",
                                                    publicAccessTab === 'zeabur'
                                            ...
                                            >
                                                ...
                                                {t('proxy.zeabur.title')}
                                                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                                                    {t('proxy.zeabur.recommended')}
                                                </span>
                                            </button>
                                            <button
                                                onClick={() => setPublicAccessTab('cloudflared')}
                                                className={cn(
                                                    "flex-1 px-3 py-2 rounded-md text-sm font-medium transition-all flex items-center justify-center gap-2",
                                                    publicAccessTab === 'cloudflared'
                                                        ? "bg-white dark:bg-base-100 text-orange-600 dark:text-orange-400 shadow-sm"
                                                        : "text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300"
                                                )}
                                            >
                                        ...
                                        {publicAccessTab === 'zeabur' && (
                                            <div className="space-y-4">
                                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                                    {t('proxy.zeabur.title_desc')}
                                                </p>

                                                {/* 实例地址 */}
                                                <div className="space-y-2">
                                                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                                        {t('proxy.zeabur.instance_url')}
                                                    </label>
                                                    <input
                                                        type="url"
                                                        value={zeaburInstanceUrl}
                                                        onChange={(e) => setZeaburInstanceUrl(e.target.value)}
                                                        disabled={zeaburStatus.connected}
                                                        placeholder={t('proxy.zeabur.instance_url_placeholder')}
                                                ...
                                                <div className="space-y-2">
                                                    <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                                                        {t('proxy.zeabur.api_key')}
                                                    </label>
                                                    <input
                                                        type="password"
                                                        value={zeaburApiKey}
                                                        onChange={(e) => setZeaburApiKey(e.target.value)}
                                                        disabled={zeaburStatus.connected}
                                                        placeholder={t('proxy.zeabur.api_key_placeholder')}
                                                        className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-base-200 text-sm font-mono disabled:opacity-60"
                                                    />
                                                </div>
                                                ...
                                                <div className="flex gap-2">
                                                    {!zeaburStatus.connected ? (
                                                        <>
                                                            <button
                                                                onClick={handleZeaburTestConnection}
                                                                disabled={zeaburLoading || !zeaburInstanceUrl.trim()}
                                                                className="px-4 py-2 rounded-lg text-sm font-medium border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-base-200 disabled:opacity-50 flex items-center gap-2"
                                                            >
                                                                {zeaburLoading ? <span className="loading loading-spinner loading-xs"></span> : null}
                                                                {t('proxy.zeabur.test_connection')}
                                                            </button>
                                                            <button
                                                                onClick={handleZeaburConnect}
                                                                disabled={zeaburLoading || !zeaburInstanceUrl.trim() || !zeaburApiKey.trim()}
                                                            ...
                                                            >
                                                                {zeaburLoading ? <span className="loading loading-spinner loading-xs"></span> : null}
                                                                {t('proxy.zeabur.connect')}
                                                            </button>
                                                        </>
                                                    ) : (
                                                        <button
                                                            onClick={handleZeaburDisconnect}
                                                            disabled={zeaburLoading}
                                                            className="px-4 py-2 rounded-lg text-sm font-medium border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 disabled:opacity-50 flex items-center gap-2"
                                                        >
                                                            {t('proxy.zeabur.disconnect')}
                                                        </button>
                                                    )}
                                                </div>
                                                ...
                                                {zeaburStatus.connected && (
                                                    <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800 space-y-3">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {zeaburStatus.version && (
                                                                <span className="text-xs text-green-600 dark:text-green-400">
                                                                    {t('proxy.zeabur.version')}: {zeaburStatus.version}
                                                                </span>
                                                            )}
                                                            <span className="text-xs text-green-600 dark:text-green-400 ml-auto">
                                                                {t('proxy.zeabur.synced_count', { count: zeaburStatus.accounts_synced })}
                                                            </span>
                                                        </div>

                                                        {/* 公网地址 */}
                                                        <div className="flex items-center gap-2">
                                                            <code className="flex-1 px-3 py-2 bg-white dark:bg-base-100 rounded text-xs font-mono text-gray-800 dark:text-gray-200 border border-green-200 dark:border-green-800 truncate">
                                                                {zeaburInstanceUrl}
                                                            </code>
                                                            <button
                                                                onClick={handleZeaburCopyUrl}
                                                                className="p-2 rounded-lg bg-green-500 text-white hover:bg-green-600 transition-colors"
                                                                title={t('proxy.zeabur.copy_url')}
                                                            >
                                                                {copied === 'zeabur-url' ? <CheckCircle size={16} /> : <Copy size={16} />}
                                                            </button>
                                                            <button
                                                                onClick={handleZeaburCopyConfig}
                                                                className="px-3 py-2 rounded-lg text-xs font-medium bg-blue-500 text-white hover:bg-blue-600 transition-colors"
                                                            >
                                                                {copied === 'zeabur-config' ? <CheckCircle size={14} /> : null}
                                                                {t('proxy.zeabur.copy_config')}
                                                            </button>
                                                        </div>
                                                    </div>
                                                )}
                                                        ...
                                                        <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-base-200 rounded-lg">
                                                            <div className="space-y-0.5">
                                                                <span className="text-sm font-medium text-gray-900 dark:text-base-content">
                                                                    {t('proxy.zeabur.auto_sync')}
                                                                </span>
                                                                <p className="text-[10px] text-gray-500 dark:text-gray-400">
                                                                    {t('proxy.zeabur.auto_sync_desc')}
                                                                </p>
                                                            </div>
                                                            <input
                                                                type="checkbox"
                                                                className="toggle toggle-sm"
                                                                checked={zeaburAutoSync}
                                                                onChange={(e) => handleZeaburSaveAutoSync(e.target.checked)}
                                                            />
                                                        </div>
                                                        ...
                                                        {zeaburAutoSync && (
                                                            <div className="flex items-center gap-3 px-3">
                                                                <span className="text-sm text-gray-600 dark:text-gray-400">
                                                                    {t('proxy.zeabur.sync_interval')}
                                                                </span>
                                                                <input
                                                                    type="number"
                                                                    min={1}
                                                                    max={60}
                                                                    value={zeaburSyncInterval}
                                                                    onChange={(e) => {
                                                                        const val = parseInt(e.target.value) || 5;
                                                                        handleZeaburSaveAutoSync(true, val);
                                                                    }}
                                                                    className="w-20 px-2 py-1 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-base-200 text-sm text-center"
                                                                />
                                                            </div>
                                                        )}

                                                        <button
                                                            onClick={handleZeaburSync}
                                                            disabled={zeaburLoading}
                                                            className="w-full px-4 py-2 rounded-lg text-sm font-medium border border-blue-200 dark:border-blue-800 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 disabled:opacity-50 flex items-center justify-center gap-2"
                                                        >
                                                            {zeaburLoading ? <span className="loading loading-spinner loading-xs"></span> : <RefreshCw size={14} />}
                                                            {t('proxy.zeabur.sync_now')}
                                                        </button>

                                                        {zeaburStatus.last_sync_time && (
                                                            <p className="text-xs text-gray-400 dark:text-gray-500 text-center">
                                                                {t('proxy.zeabur.last_sync')}: {new Date(zeaburStatus.last_sync_time * 1000).toLocaleString()}
                                                            </p>
                                                        )}
                                                    </div>
                                                )}
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                {!cfStatus.installed ? (
                                                    ...
                                                    <>
                                                        ...
                                                        {/* 启动/停止按钮 */}
                                                        <button
                                                            onClick={() => handleCfToggle(!cfStatus.running)}
                                                            disabled={cfLoading}
                                                            className={cn(
                                                                "w-full px-4 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50",
                                                                cfStatus.running
                                                        ...
                                                        >
                                                            {cfLoading ? <span className="loading loading-spinner loading-xs"></span> : <Power size={14} />}
                                                            {cfStatus.running
                                                                ? t('proxy.cloudflared.stop_tunnel')
                                                                : t('proxy.cloudflared.start_tunnel')
                                                            }
                                                        </button>
